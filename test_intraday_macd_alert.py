...
from intraday_macd_alert import (
    ...
    display_symbol,
    filter_completed_overseas_bars,
    format_report,
    fetch_name,
    filter_markets,
    load_symbols,
...
class IntradayMacdTests(unittest.TestCase):
    ...
    def test_filter_markets(self):
        ...
        self.assertEqual(filter_markets(items, "ALL"), items)

    def test_hk_incomplete_current_bar_is_excluded_at_1430(self):
        bars = pd.DataFrame(
            {
        ...
        completed = filter_completed_overseas_bars("HK:00386", bars, datetime(2026, 5, 27, 14, 30))
        self.assertEqual(len(completed), 8)
        self.assertEqual(completed.iloc[-1]["datetime"], pd.Timestamp("2026-05-27 14:00"))
    ...
    def test_close_report_contains_day_summary(self):
        ...
        self.assertAlmostEqual(result["change_today"], (31 / 28 - 1) * 100)
        ...
        self.assertIn("当日涨跌: 10.71%", report)
