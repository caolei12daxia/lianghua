...
```powershell
..\.venv\Scripts\python.exe .\macd_alert.py 000001 600519
```

第一次配置微信时，可以无论是否交叉都发送一次报告以验证 webhook：

```powershell
..\.venv\Scripts\python.exe .\macd_alert.py 000001 --notify all
```

仅预览推送内容、不请求微信：

```powershell
..\.venv\Scripts\python.exe .\macd_alert.py 000001 --notify all --dry-run
```

使用本地 CSV 验证指标逻辑，CSV 需包含 `date,close` 或 `日期,收盘` 列：

```powershell
..\.venv\Scripts\python.exe .\macd_alert.py DEMO --csv .\prices.csv --notify all --dry-run
```
...
```powershell
powershell.exe -NoProfile -Command "$env:WECOM_WEBHOOK_URL='你的webhook'; Set-Location 'C:\Users\DELL\PyCharmMiscProject\lianghua'; & '..\.venv\Scripts\python.exe' '.\macd_alert.py' 000001 600519"
```
...
通过 `--markets` 控制分析市场：`CN` 表示 A 股、`HK` 表示港股、`US` 表示美股。白天只分析 A 股和港股时使用 `--markets CN,HK`，美股单独使用 `--markets US`。

盘中每小时执行以下命令。脚本只在最新30分钟K线出现新的 MACD 金叉或死叉时发送微信提醒，并自动去重。提醒内容包含 BOLL(20,2)、RSI(14) 和 VOL 相对近5根均量的分析：

```powershell
..\.venv\Scripts\python.exe .\intraday_macd_alert.py --mode hourly --webhook-url "你的企业微信webhook"
```

收盘后执行以下命令，发送当天中文汇总分析，包含 MACD、BOLL、RSI、VOL 与一句话技术面操作观察：

```powershell
..\.venv\Scripts\python.exe .\intraday_macd_alert.py --mode close --webhook-url "你的企业微信webhook"
```
...
```powershell
..\.venv\Scripts\python.exe .\daily_macd_alert.py --mode report --webhook-url "你的企业微信webhook"
```

仅在最新日线出现新的 MACD 金叉或死叉时发送提醒：

```powershell
..\.venv\Scripts\python.exe .\daily_macd_alert.py --mode signal --webhook-url "你的企业微信webhook"
```
