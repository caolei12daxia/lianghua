...
DEFAULT_STATE_FILE = SCRIPT_DIR / ".intraday_alert_state.json"
...
def filter_completed_overseas_bars(symbol: str, bars: pd.DataFrame, now: datetime | None = None) -> pd.DataFrame:
    """Drop Yahoo's currently forming 30-minute bar using the exchange-local clock."""
    timezone = "Asia/Hong_Kong" if symbol.startswith("HK:") else "America/New_York"
    market_now = now or datetime.now(ZoneInfo(timezone)).replace(tzinfo=None)
    completed_before = market_now - timedelta(minutes=30)
    return bars[bars["datetime"] <= completed_before].reset_index(drop=True)


def fetch_bars(symbol: str, lookback_days: int) -> pd.DataFrame:
    if symbol.startswith(("HK:", "US:")):
        return filter_completed_overseas_bars(symbol, fetch_overseas_bars(symbol, "30m", "1mo"))
    response = get_with_retry(
...
def analyze(
    ...
    previous_session = bars[bars["datetime"].dt.strftime("%Y-%m-%d") < session_date]
    change = None
    if not today.empty and not previous_session.empty:
        change = (float(today.iloc[-1]["close"]) / float(previous_session.iloc[-1]["close"]) - 1) * 100
...
def format_report(results: list[dict], mode: str, trade_date: str) -> str:
    ...
    for result in results:
        ...
        if mode == "close" and result["change_today"] is not None:
            lines.append(
                f"> 当日30分钟K线: {result['bars_today']} 根 | 当日涨跌: {result['change_today']:.2f}%"
            )
    return "\n".join(lines)
...
def main() -> int:
    ...
    parser.add_argument("--markets", default="ALL", help="Market filter: CN,HK,US or ALL; for example CN,HK.")
